...
class TrieNode:
    __slots__ = ("connections", "handle", "count")
    ...
    def remove(self, data, index):
        ...
        if x in self.connections:
            if self.connections[x].remove(data, index + 1):
...
def search(node, board, coord, visited, found, root, words):
    if coord in visited:
        return
    ...
    if node.is_word and node.handle not in found:
        found.add(node.handle)
        word = words[node.handle]
        root.remove(word, 0)

    for c in adjacent(board, coord):
        search(node, board, c, visited, found, root, words)
...
class Solution:
    def findWords(self, board: List[List[str]], words: List[str]) -> List[str]:
        ...
        for row in range(N):
            for col in range(M):
                search(root, board, (row, col), set(), handles, root, words)
