DELTAS = [(1,0), (0,1), (-1,0), (0,-1)]

class TrieNode:
    __slots__ = ("connections", "is_word")

    def __init__(self):
        self.connections = {}
        self.is_word = False

    def add(self, data, index):
        if index >= len(data):
            self.is_word = True
            return

        x = data[index]
        if x not in self.connections:
            self.connections[x] = TrieNode()

        self.connections[x].add(data, index + 1)

    def step(self, value):
        if value not in self.connections:
            return None
        return self.connections[value]

def in_bounds(board, coord):
    return 0 <= coord[0] < len(board) and 0 <= coord[1] < len(board[0])

def adjacent(board, coord):
    row, col = coord
    for a, b in DELTAS:
        c = (row + a, col + b)
        if in_bounds(board, c):
            yield c

def search(node, board, coord, visited, path):
    if coord in visited:
        return

    row, col = coord
    x = board[row][col]
    node = node.step(x)

    if not node:
        return

    visited.add(coord)
    path.append(x)

    if node.is_word:
        yield "".join(path)

    for c in adjacent(board, coord):
        yield from search(node, board, c, visited, path)

    path.pop()
    visited.remove(coord)
    
class Solution:
    def findWords(self, board: List[List[str]], words: List[str]) -> List[str]:
        root = TrieNode()
        for x in words:
            root.add(x, 0)

        working = set()
        N, M = len(board), len(board[0])
        for row in range(N):
            for col in range(M):
                for word in search(root, board, (row, col), set(), []):
                    working.add(word)

        return list(working)